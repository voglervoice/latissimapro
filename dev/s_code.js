...
var s_account=s_account || "nesp-preprod";
...
s.linkInternalFilters="javascript:,nespresso.com,nesclub.nespresso.com,ecolaboration.com";
...
s._channelDomain="Social Media|facebook.com,twitter.com,youtube.com";
s._channelParameter="DFA|dfaid>Paid Search|gclid";
s._channelPattern="Emailing|EM>Banners|BA>Partners|PA>Affiliates|AF>Social Media|SM";
...
function s_doPlugins(s) {
	...
	s.channelManager('cid','','c_m','','s_dl','1');

	/* Removing Direct Load Tracking */
	if(s._channel=="Direct Load") {
	s.channelTrafic=s.getAndPersistValue(s._channel,'s_channeltrafic_p',0);
	...
	/* separating brands from Organic Search */
        if (s._channel == 'Natural Search') {
            var marque = new Array('nespr','espres','esppres','esprres','esrpe','esspre','expre','epress','nespre','nesprs','esspre','nesresso','nsspresso','nepresso','nrspresso','ecolaboration');
            var i;
            for (i = 0; i < marque.length; i++) {
                if (s._keywords.indexOf(marque[i], 0) > -1) { s._channel = "Brand SEO"; }
            }
        }
	/* separating brands from Paid Search */
        if (s._channel == 'Paid Search') {
            ...
            var i;
            for (i = 0; i < marque.length; i++) {
                if (s._keywords.indexOf(marque[i], 0) > -1) { s._channel = "Brand SEM"; }
            }
        }
        ...
        if (s._channel == 'Brand SEO') {
            s._campaign = 'N:' + s._partner + ':' + s._keywords;
        }
        if (s._channel == 'Brand SEM') {
            s._campaign = 'P:' + s._partner + ':' + s._keywords;
        }
	...
	else s._partner = s._channel + ":" + s._referringDomain;           

        }

        if (s._channel == 'Affiliates' || s._channel == 'Banners' || s._channel == 'Partners' || s._channel == 'Social Media')
        ...
        if(!s.campaign)
      	...
      	/* Clear s.campaign for DFA and collect static tracking code in eVar45  */        
        if(s.getQueryParam('dfaid')){
            s.campaign="";
            s.eVar45=s.getQueryParam('cid');        
        }


	if(!s.eVar2) 
		s.eVar2=s.getQueryParam('icid');
	s.getValOnce(s.eVar2,"s_evar2",0);
	
	if(!s.eVar28) 
		s.eVar28=s.getQueryParam('cid');
	s.getValOnce(s.eVar28,"s_evar28",0);

	
	if (!s.eVar1) 
		s.eVar1=s.getQueryParam('searchTerm');
	 ...
	 * passed in twice
	*/
	var t_search=s.getValOnce(s.eVar1,'s_evar1',0)
	if(t_search=='') {	
	...
	/* Orderid populated from purchaseid */
	if (s.purchaseID) {
		s.eVar7 = s.purchaseID;
	}
	

	/* populate dynamically prop5 with country:language:pageName */

	if ((s.prop8)&&(s.prop9)) s.prop5='D=c9+":"+c8+":"+pageName';

	/* populate dynamically prop13 with screen res:width x height */
...
if ((s_account.indexOf('nesp-5-prod',0)>-1)||(s_account.indexOf('nesp-1-prod',0)>-1)||(s_account.indexOf('nesp-3-prod',0)>-1)||(s_account.indexOf('nesp-14-prod',0)>-1)||(s_account.indexOf('nesp-2-prod',0)>-1)||(s_account.indexOf('nesp-4-prod',0)>-1)||(s_account.indexOf('nesp-9-prod',0)>-1)||(s_account.indexOf('nesp-8-prod',0)>-1)||(s_account.indexOf('nesp-12-prod',0)>-1)){

/************************** DFA VARIABLES **************************/
    if (s_account.indexOf('nesp-5-prod',0)>-1){
      var dfa_CSID='1517751'; // DFA Client Site ID Austria
		}
    if (s_account.indexOf('nesp-1-prod',0)>-1){
      var dfa_CSID='1517760'; // DFA Client Site ID Switzerland
		}
    if (s_account.indexOf('nesp-3-prod',0)>-1){
      var dfa_CSID='1517761'; // DFA Client Site ID Germany
		}
    if (s_account.indexOf('nesp-14-prod',0)>-1){
      var dfa_CSID='1517762'; // DFA Client Site ID Spain
		}
    if (s_account.indexOf('nesp-2-prod',0)>-1){
      var dfa_CSID='1517759'; // DFA Client Site ID France
		}
    if (s_account.indexOf('nesp-4-prod',0)>-1){
      var dfa_CSID='1517763'; // DFA Client Site ID Italy
		}
    if (s_account.indexOf('nesp-9-prod',0)>-1){
      var dfa_CSID='1517764'; // DFA Client Site ID Portugal
		}
    if (s_account.indexOf('nesp-8-prod',0)>-1){
      var dfa_CSID='1517758'; // DFA Client Site ID UK
		}
    if (s_account.indexOf('nesp-12-prod',0)>-1){
      var dfa_CSID='1517765'; // DFA Client Site ID USA
		}
var dfa_SPOTID='2690371'; // DFA Spotlight ID
var dfa_tEvar='eVar47'; // transfer variable, typically the "View Through" eVar.
var dfa_errorEvar='eVar48'; // DFA error tracking (optional)
var dfa_timeoutEvent='event24'; // Tracks timeouts/empty responses (optional)
...
var dfa_newRsidsProp=""; // Stores the new report suites that need the DFA tracking code. (optional)
/************************ END DFA Variables ************************/
...
s.visitorNamespace="nespresso"
s.trackingServer="metrics.nespresso.com"
s.trackingServerSecure="smetrics.nespresso.com"
